...
def write_visited_log(url):
    with open(f'Visited_frey_product_names.txt', 'a', encoding='utf-8') as file:
        file.write(f'{url}\n')


def read_log_file():
    if os.path.exists(f'Visited_frey_product_names.txt'):
        with open(f'Visited_frey_product_names.txt', 'r', encoding='utf-8') as read_file:
    ...
    for _, flinn_row in flinn_csv.iterrows():
        ...
        if pd.notna(flinn_names):
            ...
            if re.search('^\d"\sx\s\d"', str(flinn_names)):
                best_match = {
                    **flinn_row.to_dict(),
                    **{col: '' for col in frey_csv.columns}
                }
            ...
            if flinn_processed:
                try:
                    ...
                    best_match_frey = find_best_match(flinn_desc, frey_csv, 'Frey_product_name', vectorizer, title_1_vec)[1]
                    best_match = {
                        **flinn_row.to_dict(),
